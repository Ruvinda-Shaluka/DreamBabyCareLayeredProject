...
import lk.ijse.dreambabycareprojectinlayered.view.tdm.MaterialUsageTM;
...
public class MaterialUsageController implements Initializable {
    ...
    private final MaterialUsageBO materialUsageBO = (MaterialUsageBO) BOFactory.getInstance().getBO(BOFactory.BOTypes.MATERIAL_USAGE);
    ...
    private void loadMaterialIDs() {
        try {
            cmbMaterialId.setItems(FXCollections.observableArrayList(materialUsageBO.getAllMaterialIds()));
        } catch (Exception e) {
            e.printStackTrace();
            new Alert(Alert.AlertType.ERROR, "Failed to load material IDs").show();
        }
    ...
    private void loadPaymentIds() {
        try {
            cmbProductionId.setItems(FXCollections.observableArrayList(materialUsageBO.getAllProductionIds()));
        } catch (Exception e) {
            e.printStackTrace();
            new Alert(Alert.AlertType.ERROR, "Failed to load production IDs").show();
        }
    ...
    private void loadNextMaterialUsageId() {
        try {
            String nextId = materialUsageBO.generateNewId();
    ...
    private void loadMaterialUsageTableData() throws Exception {
        tblMaterialUsage.setItems(FXCollections.observableArrayList(
                materialUsageBO.loadAll()
                        .stream()
                        .map(
    ...
    public void search(KeyEvent keyEvent) {
        ...
        if (searchText.isEmpty()) {
            ...
            try {
                tblMaterialUsage.setItems(FXCollections.observableArrayList(
                        materialUsageBO.search(searchText)
                                .stream()
                                .map(
                                        materialUsageDto -> new MaterialUsageTM(
                                                materialUsageDto.getUsage_id(),
    ...
    public void saveBtnOnAction(ActionEvent actionEvent) {
        ...
        try {
            boolean isSaved = materialUsageBO.save(new MaterialUsageDto(
                    usageId,
                    productionId,
                    materialId,
                    quantityUsed
    ...
    public void updateBtnOnAction(ActionEvent actionEvent) {
        ...
        try {
            boolean isUpdated = materialUsageBO.update(new MaterialUsageDto(
                    usageId,
                    productionId,
                    materialId,
                    quantityUsed
    ...
    public void deleteBtnOnAction(ActionEvent actionEvent) {
        ...
        if (result.isPresent() && result.get() == ButtonType.YES) {
            String usageId = materialUsageIdLabel.getText();
            try {
                boolean isDeleted = materialUsageBO.delete(usageId);
                if (isDeleted) {
