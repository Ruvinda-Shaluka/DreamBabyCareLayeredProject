...
import lk.ijse.dreambabycareprojectinlayered.view.tdm.PaymentTM;
...
public class PaymentController implements Initializable {
    ...
    public void deleteBtnOnAction(ActionEvent actionEvent) {
        ...
        if (response.isPresent() && response.get() == ButtonType.YES) {
            ...
            if (selectedPayment != null) {
                try {
                    boolean isDeleted = paymentBO.delete(selectedPayment);
                    if (isDeleted) {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        try {
            cmbOrderId.setItems(FXCollections.observableArrayList(paymentBO.getAllOrderIds()));
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void loadNextId() {
        try {
            String nextId = paymentBO.generateNewId();
    ...
    private void loadPaymentTableData() throws Exception {
        tblPayment.setItems(FXCollections.observableArrayList(
                paymentBO.loadAll()
                        .stream()
                        .map(payment -> new PaymentTM(
